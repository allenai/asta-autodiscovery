...
from autodiscovery_jobs import keys
...
def _count_experiments_inline(
    ...
    """
    prefix = keys.job_key(userid, jobid, "output/")
    try:
        return sum(
            1
...
def _scan_job(
    ...
    try:
        base_path = keys.job_dir(userid, jobid)
...
def _discover_jobs(
    ...
    """Discover all (userid, jobid) pairs with a single glob listing."""
...
def _scan_all_jobs(
    ...
    try:
        ...
        raise RuntimeError(f"failed to discover jobs in {store.root_uri}: {e}") from e
