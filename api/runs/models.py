...
class AutoDiscoveryContextModel(BaseModel):
...
class ManifestModel(BaseModel):
    """Manifest written to GCS for the Asta context handoff."""
...
class DigDeeperRequestModel(BaseModel):
    """Request to initiate a 'Dig Deeper with Asta' handoff."""

    query: str = Field(..., description="User's follow-up question for Asta")


class DigDeeperResponseModel(BaseModel):
    """Response containing the Asta session URL and manifest location."""

    asta_url: str = Field(..., description="Asta chat URL for the created thread")
    manifest_gcs_uri: str = Field(..., description="GCS URI where manifest.json was saved")