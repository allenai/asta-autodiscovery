...
class BeliefTrueFalse:
        ...
        def get_mean_belief(self, prior=None, recompute=False) -> float:
            ...
            if self.mean is None or recompute:
                ...
                if prior is not None:
                    ...
                    self.mean = (self.n_true + post_alpha) / (
    ...
    def parse_response(
        ...
        return BeliefTrueFalse.DistributionFormat(
            n=n, n_true=n_true, n_false=n_false, prior_params=prior_params
        )
...
class BeliefCategorical:
    ...
    class DistributionFormat:
        ...
        def __repr__(self):
            return (
                f"BeliefCategorical.DistributionFormat(n={self.n}, definitely_true={self.definitely_true}, "
            ...
            if self.mean is None or recompute:
                ...
                if self.n > 0:
                    ...
                    self._empirical_mean = sum(
                        mean_per_category[cat] * BeliefCategorical.score_per_category[cat]
                        for cat in mean_per_category
                    )
                ...
                mean_per_category = {
                    ...
                    / (self.n + sum(self.prior_params)),
                    "uncertain": (self.uncertain + self.prior_params[2])
                    / (self.n + sum(self.prior_params)),
                ...
                if prior is not None:
                    ...
                    mean_per_category = {
                        "definitely_true": (
                            self.definitely_true + prior.definitely_true + prior.prior_params[0]
                        )
                        / (self.n + prior.n + sum(prior.prior_params)),
                        ...
                        / (self.n + prior.n + sum(prior.prior_params)),
                        "maybe_false": (
                            self.maybe_false + prior.maybe_false + prior.prior_params[3]
                        )
                        / (self.n + prior.n + sum(prior.prior_params)),
                        "definitely_false": (
                            self.definitely_false + prior.definitely_false + prior.prior_params[4]
                        )
                        / (self.n + prior.n + sum(prior.prior_params)),
                    }
                    self.mean = sum(
        ...
        def update(
            ...
            n = (
                distr.n
                if distr is not None
                else (definitely_true + maybe_true + uncertain + maybe_false + definitely_false)
            )
            if normalize:
                total = self.n + n
        ...
        def get_params(self) -> tuple[float, float, float, float, float]:
            ...
            """
            return (
                self.prior_params[0] + self.definitely_true,
    ...
    def parse_response(
        ...
        definitely_true = weight * sum(
            1 for _res in response if _res["belief"] == "definitely true"
        )
        ...
        definitely_false = weight * sum(
            1 for _res in response if _res["belief"] == "definitely false"
        )
...
class BeliefCategoricalNumeric:
    ...
    class DistributionFormat:
        ...
        def __init__(
            self,
            n: float = Field(..., description="Number of samples used to compute the distribution"),
            bucket_02: float = Field(
                ..., description="Number of responses that fall in the range [0.0, 0.2)"
            ),
            bucket_24: float = Field(
                ..., description="Number of responses that fall in the range [0.2, 0.4)"
            ),
            bucket_46: float = Field(
                ..., description="Number of responses that fall in the range [0.4, 0.6)"
            ),
            bucket_68: float = Field(
                ..., description="Number of responses that fall in the range [0.6, 0.8)"
            ),
        ...
        def __repr__(self):
            return (
                f"BeliefCategoricalNumeric.DistributionFormat(n={self.n}, bucket_02={self.bucket_02}, "
        ...
        def get_mean_belief(self, prior=None, recompute=False) -> float:
            ...
            if self.mean is None or recompute:
                ...
                if self.n > 0:
                    ...
                    self._empirical_mean = sum(
                        mean_per_category[cat] * BeliefCategoricalNumeric.score_per_category[cat]
                        for cat in mean_per_category
                    )

                mean_per_category = {
                    "0-0.2": (self.bucket_02 + self.prior_params[0])
                    / (self.n + sum(self.prior_params)),
                    "0.2-0.4": (self.bucket_24 + self.prior_params[1])
                    / (self.n + sum(self.prior_params)),
                    "0.4-0.6": (self.bucket_46 + self.prior_params[2])
                    / (self.n + sum(self.prior_params)),
                    "0.6-0.8": (self.bucket_68 + self.prior_params[3])
                    / (self.n + sum(self.prior_params)),
                    "0.8-1.0": (self.bucket_810 + self.prior_params[4])
                    / (self.n + sum(self.prior_params)),
                }
                self.mean = sum(
                    mean_per_category[cat] * BeliefCategoricalNumeric.score_per_category[cat]
        ...
        def update(
            ...
            n = (
                distr.n
                if distr is not None
                else (bucket_02 + bucket_24 + bucket_46 + bucket_68 + bucket_810)
            )
            if normalize:
                total = self.n + n
        ...
        def get_params(self) -> tuple[float, float, float, float, float]:
            ...
            """
            return (
                self.prior_params[0] + self.bucket_02,
    ...
    class ResponseFormat(BaseModel):
        ...
        belief: str = Field(
            ...,
            description="Belief about the hypothesis being true",
            choices=["0-0.2", "0.2-0.4", "0.4-0.6", "0.6-0.8", "0.8-1.0", "cannot comment"],
        )
...
class BeliefGauss:
    ...
    class DistributionFormat:
        def __init__(
            ...
            **kwargs,
            ...
            self._empirical_mean, self._empirical_stddev = fuse_gaussians(
                self.samples["means"], self.samples["stddevs"]
            )
            self.mean = mean
            self.stddev = stddev
            self.prior_params = (
        ...
        def get_mean_belief(self, prior=None, recompute=False) -> float:
            ...
            if recompute:
                ...
                self.mean, self.stddev = fuse_gaussians(
                    [self.prior_params[0]] + self.samples["means"],
        ...
        def update(
            ...
            if distr is not None:
                ...
                self._empirical_mean, self._empirical_stddev = fuse_gaussians(
                    [self._empirical_mean, distr._empirical_mean],
                    [self._empirical_stddev, distr._empirical_stddev],
                )
                self.mean, self.stddev = fuse_gaussians(
                ...
                self._empirical_mean, self._empirical_stddev = fuse_gaussians(
                    [self._empirical_mean] + means, [self._empirical_stddev] + stddevs
                )
                self.mean, self.stddev = fuse_gaussians(
    ...
    class ResponseFormat(BaseModel):
        ...
        belief_mean: float = Field(
            ..., description="Mean of the belief distribution", ge=0.0, le=1.0
        )
        belief_stddev: float = Field(
            ...,
            description="Standard deviation of the belief distribution",
            ge=0.0,
            le=float("inf"),
        )
    ...
    def parse_response(
        ...
        mean, stddev = fuse_gaussians(
            [prior_params[0]] + means, [prior_params[1]] + stddevs, weight=weight
        )

        return BeliefGauss.DistributionFormat(
            n=n,
            mean=mean,
            stddev=stddev,
            prior_params=prior_params,
            samples={"means": means, "stddevs": stddevs},
            weight=weight,
        )
    ...
    def kl_divergence(
        ...
        return 0.5 * (
            np.log(stddev2**2 / stddev1**2) + (stddev1**2 + (mean1 - mean2) ** 2) / stddev2**2 - 1
        )
...
def get_belief(
    ...
    if belief_cls is None:
        raise ValueError(
            f"Unknown belief_mode '{belief_mode}'; expected one of {list(BELIEF_MODE_TO_CLS.keys())}"
        )
    ...
    system_prompt = {"role": "system", "content": " ".join(_system_msgs)}
    ...
    for attempt in range(n_retries):
        try:
            ...
            distribution = belief_cls.parse_response(
                response, weight=weight, **prior_params_or_none
            )
        ...
        evidence_msg = [
            {
                ...
                "content": get_context_string(
                    query, code_output, analysis, review, include_code_output=True
                ),
            }
        ]
