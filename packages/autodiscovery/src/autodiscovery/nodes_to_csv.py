...
def nodes_to_csv(nodes_or_json_path, out_fpath):
    ...
    for node in mcts_nodes:
        ...
        if prior_mean is not None and posterior_mean is not None:
            csv_node["belief_dir"] = (
                ...
                else ("same" if posterior_mean == prior_mean else "pos")
            )

        csv_node["hypothesis"] = node["hypothesis"]
        experiment_plan = node["experiment_plan"]
        csv_node["experiment_plan"] = (
            f"Objective: {experiment_plan.get('objective', 'N/A')}\n\n"
    ...
    csv_list.sort(
        key=lambda x: x["belief_change"] if x["belief_change"] is not None else float("-inf"),
        reverse=True,
    )
