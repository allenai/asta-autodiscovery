...
**What it deletes:**
- Files matching: `gs://ai2-autodiscovery/users/*/jobs/*/data/*`
...
This script is deployed as a Cloud Run Job triggered by Cloud Scheduler in the `ai2-aristo` GCP project.
...
1. **Build and push container:**
   ```bash
   docker build --platform linux/amd64 -t gcr.io/ai2-aristo/autodiscovery-dataset-cleanup -f scripts/Dockerfile .
   docker push gcr.io/ai2-aristo/autodiscovery-dataset-cleanup
   ```
...
2. **Create Cloud Run Job:**
   ...
   gcloud run jobs create autodiscovery-dataset-cleanup \
     --image gcr.io/ai2-aristo/autodiscovery-dataset-cleanup \
     --region us-west1 \
     --command "uv" \
...
3. **Schedule with Cloud Scheduler:**
   ```bash
   project_number=$(gcloud projects describe ai2-aristo --format="value(projectNumber)")
